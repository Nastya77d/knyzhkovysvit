...
function exportData() {
    const data = {
        // 1. Активне читання та бібліотека
        ...
        // 2. Статистика та цитати
        dailyLogs: JSON.parse(localStorage.getItem('dailyLogs')) || {},
        allQuotes: JSON.parse(localStorage.getItem('allQuotes')) || {},

        // 3. ПЛАНИ (тепер точно зберігаються!)
        tbrLibrary: JSON.parse(localStorage.getItem('tbrLibrary')) || [],
        unreadLibrary: JSON.parse(localStorage.getItem('unreadLibrary')) || [],
        
        // Для сумісності зі старими версіями про всяк випадок
        tbrList: JSON.parse(localStorage.getItem('tbrLibrary')) || []
    };
...
function importData(event) {
    ...
    reader.onload = function(e) {
        try {
            ...
            if (confirm("Відновити всі списки та плани?")) {
                // 1. Бібліотека та активні книги
                if (data.currentlyReading) localStorage.setItem('currentlyReading', JSON.stringify(data.currentlyReading));
                if (data.myLibrary) localStorage.setItem('myLibrary', JSON.stringify(data.myLibrary));
                if (data.goldLibrary) localStorage.setItem('goldLibrary', JSON.stringify(data.goldLibrary));
                if (data.allQuotes) localStorage.setItem('allQuotes', JSON.stringify(data.allQuotes));
                if (data.dailyLogs) localStorage.setItem('dailyLogs', JSON.stringify(data.dailyLogs));

                // 2. ПЛАНИ (Виправляємо конфлікт назв tbrList vs tbrLibrary)
                const plans = data.tbrLibrary || data.tbrList; // беремо те, що є у файлі
                if (plans) {
                    localStorage.setItem('tbrLibrary', JSON.stringify(plans)); // зберігаємо як tbrLibrary
                }
                
                if (data.unreadLibrary) localStorage.setItem('unreadLibrary', JSON.stringify(data.unreadLibrary));
                
                alert("Дані успішно завантажені! Плани тепер на місці.");
                location.reload();
            }
        } catch (err) {
            alert("Помилка у форматі файлу.");
        }
    };
...
async function handleQuoteScan(file) {
    ...
    // 1. Тимчасово ховаємо вікно "Нова цитата"
    const quoteModal = document.querySelector('.add-quote-modal');
    if (quoteModal) quoteModal.style.visibility = 'hidden'; 
    ...
    reader.onload = function(e) {
        ...
        // ВСТАНОВЛЮЄМО Z-INDEX 2000000
        modal.style.cssText = 'position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.98); display:flex; flex-direction:column; justify-content:center; align-items:center; z-index:2000000; padding:10px; box-sizing:border-box;';
        
        modal.innerHTML = `
            <div style="position:relative; width:100%; display:flex; justify-content:center; overflow:hidden; margin-bottom:15px;">
                <canvas id="perspective-canvas" style="border:2px solid #d4af37; touch-action: none; background:#000;"></canvas>
            </div>
            <p style="color:#d4af37; margin:10px 0; font-size:0.9rem; font-weight:bold; text-shadow: 1px 1px 2px #000;">Тягни за кути, щоб виділити текст</p>
            <div style="display:flex; gap:15px; width:100%; max-width:400px;">
                <button id="cancel-scan-btn" style="flex:1; padding:15px; background:#444; color:white; border:none; border-radius:12px; font-weight:bold; cursor:pointer;">❌</button>
                <button id="start-scan-btn" style="flex:2; padding:15px; background:#d4af37; color:black; border:none; border-radius:12px; font-weight:bold; font-size:1rem; cursor:pointer;">✅ Сканувати</button>
            </div>
        `;
        ...
        document.getElementById('cancel-scan-btn').onclick = () => {
            modal.remove();
            if (quoteModal) quoteModal.style.visibility = 'visible';
        };
        ...
        originalImage.onload = function() {
            ...
            const displayHeight = window.innerHeight * 0.65;
            ...
            document.getElementById('start-scan-btn').onclick = () => {
                processPerspectiveCrop(canvas);
                // Після обробки (у наступній функції) ми знову покажемо вікно цитат
            };
        };
    };
...
async function processPerspectiveCrop(canvas) {
    ...
    // 1. Попередньо готуємо статус (якщо вікно ще не з'явилося, воно оновиться пізніше)
    ...
    resultCanvas.toBlob(async (blob) => {
        // 2. ВИДАЛЯЄМО модалку сканера
        const perspectiveModal = document.getElementById('perspective-modal');
        if (perspectiveModal) perspectiveModal.remove();

        // 3. ПОВЕРТАЄМО видимість вікна "Нова цитата"
        const quoteModal = document.querySelector('.add-quote-modal');
        if (quoteModal) {
            quoteModal.style.visibility = 'visible';
            // Оновлюємо статус і посилання на елементи, бо вони могли змінити стан
            const currentStatus = document.getElementById('scan-status');
            const currentTextArea = document.getElementById('quote-text');
            if (currentStatus) currentStatus.innerHTML = "🔍 <b>Обробка тексту...</b>";
        }
        ...
        try {
            ...
            // 4. Записуємо результат у textarea
            const currentTextArea = document.getElementById('quote-text');
            if (currentTextArea) {
                currentTextArea.value = text.replace(/\n/g, ' ').replace(/\s+/g, ' ').trim();
            }

            const currentStatus = document.getElementById('scan-status');
            if (currentStatus) currentStatus.innerText = "✨ Готово!";

        } catch (err) {
            const currentStatus = document.getElementById('scan-status');
            if (currentStatus) currentStatus.innerText = "❌ Помилка сканування";
            console.error(err);
        }
    }, 'image/jpeg', 1.0);
}
...
function openAddQuoteModal(bookTitle) {
    ...
    modal.style.cssText = 'position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.9); display:flex; justify-content:center; align-items:center; z-index:999999; padding:20px; box-sizing:border-box;';
    
    modal.innerHTML = `
        <div style="background:#1a1a1a; padding:25px; border:1px solid #d4af37; border-radius:15px; width:100%; max-width:400px; text-align:center; position:relative; box-sizing:border-box;">
            <h3 style="color:#d4af37; margin-top:0;">📝 Нова цитата</h3>
            <p style="color:#888; font-size:0.8rem; margin-bottom:15px;">${bookTitle}</p>
            
            <input type="number" id="quote-page" placeholder="№ Сторінки" style="width:100%; padding:10px; margin-bottom:10px; background:#2a2a2a; color:white; border:1px solid #444; border-radius:8px; box-sizing:border-box;">
            
            <textarea id="quote-text" placeholder="Текст цитати з'явиться тут..." style="width:100%; height:120px; padding:10px; margin-bottom:15px; background:#2a2a2a; color:white; border:1px solid #444; border-radius:8px; font-family:serif; resize:none; box-sizing:border-box;"></textarea>
            
            <div id="scan-status" style="font-size:0.8rem; color:#d4af37; margin-bottom:15px; min-height:1.2rem;"></div>

            <div style="display:grid; grid-template-columns: 1fr 1fr; gap:10px;">
                <button id="trigger-scan-hidden" style="background:#2a2a2a; color:#d4af37; border:1px solid #d4af37; padding:12px; border-radius:8px; cursor:pointer;">📷 Скан</button>
    ...
    // Створюємо інпут "в повітрі" - його НЕМАЄ в HTML коді модалки
    const scanBtn = modal.querySelector('#trigger-scan-hidden');
    scanBtn.onclick = function() {
        let input = document.getElementById('temp-scan-input');
        if (!input) {
            input = document.createElement('input');
            input.type = 'file';
            input.id = 'temp-scan-input';
            input.accept = 'image/*';
            input.capture = 'camera';
            input.style.display = 'none'; // Повне приховування
            input.onchange = (e) => handleQuoteScan(e.target.files[0]);
