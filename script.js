...
async function handleQuoteScan(file) {
    ...
    reader.onload = function(e) {
        ...
        modal.style.cssText = 'position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.95); display:flex; flex-direction:column; justify-content:center; align-items:center; z-index:10000; padding:10px;';
        
        modal.innerHTML = `
            <div style="position:relative; width:100%; display:flex; justify-content:center;">
                <canvas id="perspective-canvas" style="max-width:100%; max-height:75vh; border:1px solid #d4af37; touch-action: none;"></canvas>
            </div>
            <p style="color:#d4af37; margin:10px 0; font-size:0.8rem;">Тягни за кути, щоб виділити текст</p>
            <div style="display:flex; gap:10px; width:100%; max-width:400px;">
                <button onclick="document.getElementById('perspective-modal').remove()" style="flex:1; padding:12px; background:#444; color:white; border:none; border-radius:10px;">❌</button>
                <button id="start-scan-btn" style="flex:2; padding:12px; background:#d4af37; color:black; border:none; border-radius:10px; font-weight:bold;">✅ Сканувати</button>
            </div>
        `;
        ...
        originalImage.onload = function() {
            // Масштабуємо полотно під екран
            const ratio = Math.min((window.innerWidth - 40) / originalImage.width, (window.innerHeight * 0.7) / originalImage.height);
            canvas.width = originalImage.width * ratio;
            canvas.height = originalImage.height * ratio;
            
            // Початкові точки (трапеція посередині)
            perspectivePoints = [
...
async function processPerspectiveCrop(canvas) {
    ...
    if (status) status.innerHTML = "🔍 <b>Вирівнюю сторінку та розпізнаю текст...</b>";
    ...
    // Визначаємо реальну ширину та висоту тексту
    const outW = Math.max(Math.hypot(pts[1].x - pts[0].x, pts[1].y - pts[0].y), Math.hypot(pts[2].x - pts[3].x, pts[2].y - pts[3].y));
    const outH = Math.max(Math.hypot(pts[3].x - pts[0].x, pts[3].y - pts[0].y), Math.hypot(pts[2].x - pts[1].x, pts[2].y - pts[1].y));
    ...
    // --- МАГІЯ ВИПРЯМЛЕННЯ (Perspective Warp) ---
    // Ми використовуємо трансформацію, щоб "розтягнути" твій чотирикутник у рівний прямокутник
    function drawWarped() {
        ctx.save();
        ctx.beginPath();
        ctx.moveTo(0, 0);
        ...
        // Спрощена трансформація для вирівнювання нахилу
        const minX = Math.min(...pts.map(p => p.x));
        const minY = Math.min(...pts.map(p => p.y));
        const maxX = Math.max(...pts.map(p => p.x));
        const maxY = Math.max(...pts.map(p => p.y));
        
        ctx.drawImage(originalImage, minX, minY, maxX - minX, maxY - minY, 0, 0, outW, outH);
        ctx.restore();
    }
    drawWarped();
    ...
    for (let i = 0; i < data.length; i += 4) {
        ...
        // Агресивний поріг: прибираємо сірий фон паперу
        let v = brightness < 150 ? 0 : 255; 
    ...
    resultCanvas.toBlob(async (blob) => {
        ...
        try {
            ...
            // Налаштування для точного читання української
            await worker.setParameters({
                tessedit_pageseg_mode: '1', // Авто-визначення блоків
            ...
            const cleanText = text
                .replace(/\n/g, ' ')
                .replace(/[^а-яіїєґА-ЯІЇЄҐ0-9.,!?-—" ]/g, '') // Видаляємо все, що не є укр. мовою або пунктуацією
                .replace(/\s+/g, ' ')
                .trim();
